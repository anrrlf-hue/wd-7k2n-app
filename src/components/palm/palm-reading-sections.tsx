      ...
      <div className="space-y-3">
        {items.map(item => (
          <div key={item.topic} className="rounded-xl border border-border p-4">
            <h3 className="text-sm font-semibold">{item.topic === "관계에서 감정이 작용하는 정도" ? "관계와 의사 표현" : item.topic}</h3>
            <p className="mt-2 whitespace-pre-line text-sm leading-7 text-muted-foreground">{item.text}</p>
          </div>
        ))}
      </div>
    </ReportSection>
