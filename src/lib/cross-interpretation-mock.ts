...
export function buildMockCrossInterpretation(
  ...
  palm: PalmFacts,
  personality?: { facts: SajuFacts | null; check: PersonalityCheckFacts | null; mbti: MbtiSelfReport | null },
): CrossInterpretation {
  ...
  const personalityNote =
    personality?.facts && (personality.check || personality.mbti)
      ? buildPersonalityComparisonText(personality.facts, personality.check, personality.mbti)
      : null;
