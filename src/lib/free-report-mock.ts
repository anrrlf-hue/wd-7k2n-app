...
export function buildFreeSajuReport(
  ...
  const seedFor = (i: number) => baseSeed + i * 7;
    ...
    activeCompare === "output"
      ...
      : activeCompare === "peer"
        ...
        : `식상과 비겁이 뚜렷하게 우세하지 않아서, 벌어들이는 힘은 재성·용신 쪽에서 더 크게 작동하는 편이에요. ` +
          ...
          `이 해석은 식상 ${outputStarCount}개·비겁 ${peerStarCount}개, 용신(${yongsin.join(", ") || "특이 없음"})을 근거로 했어요.`;

  // ⑤ 돈을 지키는 방식
  const keepingStyle =
    dayStrength === "strong"
      ? `자기 기준이 뚜렷해서 웬만해선 안 흔들려요. 다만 그 확신이 지나치면 주변 조언을 안 듣고 밀어붙이다 지키는 힘 자체를 스스로 깎아먹기도 해요. ` +
        `결정하기 전에 딱 한 번만 다른 사람 의견을 들어보는 걸 시도해보면 좋아요. "이미 마음먹었는데 왜 물어봐" 싶을 때가 많다면 특히 더요. ` +
  ...
  const opportunityStyle = compose(seedFor(13), {
    ...
    scene:
      gilsin.length > 0
        ? `${gilsin.join(", ")} 같은 길신도 있어서, 결정적 순간에 예상치 못한 도움을 받는 경우가 실제로 있을 수 있어요.`
  ...
  const evidenceExplainer =
    `이 결과는 태어난 날의 하늘 기운(일간) ${dayStemKo}(${dayElement})을 중심에 두고, 원국 전체 여덟 글자의 오행 균형과 십성(재성·비겁·식상·관성·인성) 개수, 격국(${geukguk})과 용신(${yongsin.join(", ") || "특이 없음"})까지 함께 봐서 만들었어요. ` +
    `쉽게 말하면, "나"를 뜻하는 글자(일간)가 얼마나 힘이 있는지(${dayStrengthShort(dayStrength)}), 그 주변에 돈(재성)·경쟁(비겁)·활동(식상)·조직(관성)·정보(인성)를 뜻하는 글자가 몇 개씩 있는지, 그리고 12운성으로 그 힘이 어느 시기에 정점을 찍는지를 함께 읽은 거예요. ` +
    `${daeunFlowNote} ` +
