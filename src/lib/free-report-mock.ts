...
export function buildFreeSajuReport(facts: SajuFacts, personality?: PersonalityInput): FreeSajuReport {
  ...
  const snapshot: ReportParagraph = {
    text:
      ...
      `${dayStrength === "strong" ? "스스로 기준을 세우고 밀고 나가는 힘이 강한 편입니다" : dayStrength === "weak" ? "주변의 도움과 환경을 활용하면서 방향을 잡는 편입니다" : "내 기준과 주변 상황을 함께 보며 균형을 잡는 편입니다"}. ` +
      `일과 돈에서는 ${activeCompare === "output" ? "생각을 표현하고 결과물로 만들어 보여줄 때" : activeCompare === "peer" ? "직접 실행하며 내 몫을 만들어갈 때" : "구상과 실행을 함께 엮을 때"} 강점이 드러납니다.`,
  ...
  const keepingStyle: ReportParagraph =
    dayStrength === "strong"
      ? {
          text:
            `돈을 지킬 때는 자기 기준을 쉽게 바꾸지 않는 편입니다. 마음이 움직이는 제안을 만나도 ‘내가 처음 정한 기준에 맞는가’를 확인하려는 성향이 있습니다. ` +
            `다만 확신이 강해질수록 다른 관점을 놓칠 수 있어, 큰 결정일수록 반대 의견 하나는 확인하는 편이 좋습니다.`,
          evidence: `일간 ${dayStemKo}(${dayStrengthShort(dayStrength)}), 격국 ${geukguk}`,
        }
      : dayStrength === "weak"
        ? {
            text: `돈을 지킬 때는 혼자 밀어붙이기보다 믿을 만한 조언이나 정리된 기준을 활용하는 편입니다. 다만 남의 말을 그대로 따르기보다, 왜 그 선택을 하는지 스스로 납득해야 안정적으로 유지됩니다.`,
            evidence: `일간 ${dayStemKo}(${dayStrengthShort(dayStrength)})`,
          }
        : {
            text: `돈을 지킬 때는 한쪽으로 고집하기보다 상황을 보며 기준을 조정하는 편입니다. 무엇은 바꿔도 되고 무엇은 끝까지 지킬지 구분할수록 판단이 안정됩니다.`,
  ...
  const peopleAndMoney: ReportParagraph =
    socialCompare === "officer"
      ? {
          text:
            `사람과 돈이 얽힌 일에서는 역할과 약속을 분명히 해야 마음이 놓이는 편입니다. ` +
            `누가 무엇을 맡고 언제까지 할지 기준이 명확할수록 편하고, 맡은 뒤에는 내 방식대로 풀어갈 여지가 있어야 힘이 납니다.`,
          evidence: `관성(편관+정관) ${officerStarCount}개`,
        }
      : socialCompare === "resource"
        ? {
            text:
              `사람과 돈이 얽힌 일에서는 경험 있는 사람의 설명이나 주변의 도움을 잘 활용하는 편입니다. ` +
              `다만 들은 조언을 그대로 따르기보다 내 상황에 맞는 부분만 골라 쓸 때 판단력이 살아납니다.`,
            evidence: `인성(편인+정인) ${resourceStarCount}개`,
          }
        : {
            text: `사람과 돈이 얽힌 일에서는 약속과 책임도 중요하고, 상대에게 배우거나 도움받는 것도 중요하게 보는 편입니다. 함께할 때 역할을 먼저 맞추고 필요한 도움을 구체적으로 정할수록 관계가 편해집니다.`,
            evidence: `관성 ${officerStarCount}개·인성 ${resourceStarCount}개의 균형`,
          };

  const decisionStyle: ReportParagraph = {
    text: dayStrength === "neutral"
      ? "결정을 내릴 때 자기 기준과 주변 상황을 함께 보는 편입니다. 무조건 혼자 정하거나 무조건 따르기보다, 지킬 기준과 조정할 부분을 나눠 판단합니다."
      : dayStrength === "strong"
        ? "결정을 내릴 때는 스스로 납득할 기준이 먼저 서야 움직이는 편입니다. 선택지가 많아도 중요한 조건을 정하면 빠르게 방향을 잡을 수 있지만, 마음이 기운 뒤에는 다른 의견을 놓칠 수 있습니다."
        : "결정을 내릴 때는 필요한 정보와 주변의 도움을 충분히 모은 뒤 움직이는 편입니다. 여러 의견을 잘 듣는 장점이 있지만, 기준이 너무 많아지면 결정이 늦어질 수 있습니다.",
