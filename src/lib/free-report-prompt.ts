...
3. 사주 성향은 "이 해석에서는 ~ 경향으로 읽습니다"로 한정하세요. 과거 경험, 주변 평가, 돈이 실제 모이거나 새는 상태를 확인한 것처럼 서술하지 마세요. 핵심 일 해석을 모든 일/수입/조직/독립 섹션에 일관되게 적용하세요. 역할·규칙과 실행 자율성의 공존을 설명하되 출생정보만으로 직장/사업/프리랜서를 권하지 마세요. 이 리포트는 현재 직업, 소득, 지출, 자산, 부채, 재무 목표를 절대 묻지도 언급하지도 않습니다. 오직 태어난 날짜(사주 원국)만 근거로 삼으세요. 손금·자기보고 성향정보는 이 리포트가 아니라 별도 단계에서 비교합니다 — 여기서는 다루지 마세요.
...
6. 같은 문장 패턴을 반복하지 마세요. 섹션마다 도입 방식(장면 묘사로 시작 / 해석 범위를 밝히며 시작)을 바꾸세요. 전통 해석이라는 범위는 유지하되, 본문을 "해석하지 않습니다 / 비교 불가 / 측정한 값이 아닙니다" 같은 거절·면책 문장으로 채우지 마세요. 실제 근거에서 읽을 수 있는 성향과 생활 속 질문을 구체적으로 설명하고, 계산 방법과 해석의 한계는 evidence에 담으세요. 보이지 않는 사실을 지어내 빈칸을 채우라는 뜻은 아닙니다.
